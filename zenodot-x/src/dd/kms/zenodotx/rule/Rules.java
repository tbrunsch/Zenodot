...
public class Rules
{
	...
	public static <I, O, S extends GrammarSettings> OrRule<I, O, S> or(Rule<I, O, S>... alternatives) {
		return or(Arrays.asList(alternatives));
	}

	public static <I, O, S extends GrammarSettings> OrRule<I, O, S> or(List<Rule<I, O, S>> alternatives) {
		OrRule<I, O, S> orRule = new OrRuleImpl<>();
		orRule.setAlternatives(alternatives);
		return orRule;
	}

	public static <IO, S extends GrammarSettings> Rule<IO, IO, S> repeat(Rule<IO, IO, S> ruleToRepeat) {
		return repeat(null, ruleToRepeat);
	}

	public static <IO, S extends GrammarSettings> Rule<IO, IO, S> repeat(String name, Rule<IO, IO, S> ruleToRepeat) {
	...
	public static <IO, S extends GrammarSettings> SimpleRule<IO, IO, S> character(char character) {
		return new CharacterRule<>(character);
	}

	public static <IO, S extends GrammarSettings> SimpleRule<IO, IO, S> space() {
		return new SpaceRule<>();
	}

	public static <IO, S extends GrammarSettings> SimpleRule<IO, IO, S> empty() {
