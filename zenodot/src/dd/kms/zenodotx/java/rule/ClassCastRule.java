...
public class ClassCastRule extends AbstractRule<Class<?>, InstanceParseResult, JavaSettings> implements CompoundRule<Class<?>, InstanceParseResult, JavaSettings>
{
	...
	public InstanceParseResult parse(Class<?> targetClass, JavaSettings settings, Parser<JavaSettings> parser) throws SyntaxException, EvaluationException, SemanticException, Parser.EventResultException {
		...
		return new CastParseResult(targetClass, expression, settings);
	}
	...
	private static class CastParseResult implements InstanceParseResult
	{
		...
		private CastParseResult(Class<?> targetClass, InstanceParseResult expression, JavaSettings settings) throws EvaluationException {
			this.targetClass = targetClass;
			this.expression = expression;
			this.evaluatedObject = evaluate(expression.getEvaluatedResult(), settings);
		}
		...
		public ObjectInfo evaluate(JavaSettings settings) throws EvaluationException {
			ObjectInfo expressionInfo = this.expression.evaluate(settings);
			return evaluate(expressionInfo, settings);
		}

		private ObjectInfo evaluate(ObjectInfo expressionInfo, JavaSettings settings) throws EvaluationException {
